...
{
    return mavlink_msg_command_ack_pack(system_id, component_id, msg, (uint16_t )(command_ack->command), (uint8_t )(command_ack->result));
}
...
{
    return mavlink_msg_command_ack_pack_chan(system_id, component_id, chan, msg, (uint16_t )(command_ack->command), (uint8_t )(command_ack->result));
}
...
#else
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_COMMAND_ACK, (const char *)command_ack, MAVLINK_MSG_ID_COMMAND_ACK_LEN);
#endif
#endif
}
