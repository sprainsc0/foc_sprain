...
{
    return mavlink_msg_fence_status_pack(system_id, component_id, msg, (uint8_t )(fence_status->breach_status), (uint16_t )(fence_status->breach_count), (uint8_t )(fence_status->breach_type), (uint32_t )(fence_status->breach_time));
}
...
{
    return mavlink_msg_fence_status_pack_chan(system_id, component_id, chan, msg, (uint8_t )(fence_status->breach_status), (uint16_t )(fence_status->breach_count), (uint8_t )(fence_status->breach_type), (uint32_t )(fence_status->breach_time));
}
...
#else
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_FENCE_STATUS, (const char *)fence_status, MAVLINK_MSG_ID_FENCE_STATUS_LEN);
#endif
#endif
}
