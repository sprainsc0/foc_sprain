...
{
    return mavlink_msg_scaled_pressure_pack(system_id, component_id, msg, (uint32_t )(scaled_pressure->time_boot_ms), (float )(scaled_pressure->press_abs), (float )(scaled_pressure->press_diff), (int16_t )(scaled_pressure->temperature));
}
...
{
    return mavlink_msg_scaled_pressure_pack_chan(system_id, component_id, chan, msg, (uint32_t )(scaled_pressure->time_boot_ms), (float )(scaled_pressure->press_abs), (float )(scaled_pressure->press_diff), (int16_t )(scaled_pressure->temperature));
}
...
#else
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_SCALED_PRESSURE, (const char *)scaled_pressure, MAVLINK_MSG_ID_SCALED_PRESSURE_LEN);
#endif
#endif
}
