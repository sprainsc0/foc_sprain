...
{
    return mavlink_msg_mission_count_pack(system_id, component_id, msg, (uint8_t )(mission_count->target_system), (uint8_t )(mission_count->target_component), (uint16_t )(mission_count->count));
}
...
{
    return mavlink_msg_mission_count_pack_chan(system_id, component_id, chan, msg, (uint8_t )(mission_count->target_system), (uint8_t )(mission_count->target_component), (uint16_t )(mission_count->count));
}
...
#else
    _mav_finalize_message_chan_send(chan, MAVLINK_MSG_ID_MISSION_COUNT, (const char *)mission_count, MAVLINK_MSG_ID_MISSION_COUNT_LEN);
#endif
#endif
}
